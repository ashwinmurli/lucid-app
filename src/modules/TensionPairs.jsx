  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
        ...
        border: `1px solid ${isSelected ? "rgba(212,115,74,0.15)" : colors.border}`,
      ...
      <div style={{ padding: "14px 16px 12px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div style={{ fontSize: 18, fontWeight: 300, color: colors.text, lineHeight: 1.4, letterSpacing: "-0.01em" }}>
          ...
          <span style={{ color: "rgba(61,56,48,0.2)" }}>, but never </span>
          <span style={{ color: "rgba(61,56,48,0.35)" }}>{pair.excess.toLowerCase()}</span>
        </div>
        {!isLocked && (
      ...
      <div style={{ padding: "8px 16px", display: "flex", alignItems: "flex-start", gap: 8 }}>
        <div style={{ width: 4, height: 4, borderRadius: "50%", marginTop: 4, flexShrink: 0, background: colors.accent, boxShadow: "0 0 4px rgba(212,115,74,0.3)" }} />
        <div style={{ fontFamily: fonts.pixel, fontSize: 10, color: "rgba(212,115,74,0.45)", lineHeight: 1.5 }}>{pair.reason}</div>
      </div>
    </div>
  );
}
        ...
        <div style={{ maxWidth: 520, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 12px", borderRadius: 4, marginBottom: 16, background: "rgba(61,56,48,0.04)", boxShadow: "0 1px 2px rgba(0,0,0,0.03) inset, 0 1px 0 rgba(255,255,255,0.5)" }}>
              ...
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(61,56,48,0.3)" }}>TENSIONS LOCKED</span>
            </div>
    ...
    <div style={{ padding: "40px 48px 60px", animation: `promptIn 0.5s ${ease} both` }}>
      <div style={{ maxWidth: 520, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          <p style={{ fontSize: 12, fontWeight: 400, color: "rgba(61,56,48,0.3)", lineHeight: 1.6 }}>Toggle three tensions that feel most true.</p>
        </div>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ background: colors.recess, borderRadius: "6px 6px 0 0", border: `1px solid ${colors.border}`, borderBottom: "none", overflow: "hidden" }}>
            ...
            <div style={{ margin: "0 16px", padding: "6px 0", borderTop: "1px solid rgba(61,56,48,0.04)", borderBottom: "1px solid rgba(61,56,48,0.04)", fontSize: 8, fontWeight: 700, letterSpacing: "0.1em", color: "rgba(61,56,48,0.12)", textTransform: "uppercase" }}>BUT NEVER</div>
            <div style={{ padding: "10px 16px 14px" }}><input value={customExcess} onChange={(e) => setCustomExcess(e.target.value)} placeholder="Excess..." style={{ width: "100%", background: "transparent", border: "none", fontSize: 18, fontWeight: 300, color: "rgba(61,56,48,0.5)", outline: "none", fontFamily: fonts.primary, letterSpacing: "-0.01em" }} onKeyDown={(e) => { if (e.key === "Enter" && customQuality.trim() && customExcess.trim()) addCustomPair(); }} /></div>
            <div style={{ height: 1, background: colors.border }} />
            <TransportBtn onClick={addCustomPair} disabled={!customQuality.trim() || !customExcess.trim()}>ADD PAIR</TransportBtn>
          </div>
          <div style={{ background: colors.recess, borderRadius: "0 0 6px 6px", border: `1px solid ${colors.border}`, borderTop: "1px solid rgba(61,56,48,0.04)", padding: "6px 8px" }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 5 }}>
              ...
              <div style={{ display: "flex", borderRadius: 3, flexShrink: 0, background: "rgba(61,56,48,0.04)", boxShadow: "0 1px 3px rgba(0,0,0,0.04) inset, 0 1px 0 rgba(255,255,255,0.4)", padding: 2, marginTop: 2 }}>
                {Object.entries(MODES).map(([key, m]) => (<button key={key} onClick={() => setAiMode(key)} onMouseEnter={() => setHoveredAiMode(key)} onMouseLeave={() => setHoveredAiMode(null)} style={{ width: 28, height: 22, borderRadius: 2, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: fonts.primary, fontSize: 7, fontWeight: 700, letterSpacing: "0.04em", color: aiMode === key ? "#EDE9E1" : "rgba(61,56,48,0.2)", background: aiMode === key ? colors.accent : "transparent", boxShadow: aiMode === key ? "0 1px 3px rgba(0,0,0,0.12), 0 1px 0 rgba(255,180,140,0.1) inset" : "none", transition: "all 0.15s ease" }}>{m.key}</button>))}
              </div>
            </div>
          </div>
        </div>
