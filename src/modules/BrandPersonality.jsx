...
function PersonalityLucyScreen({ mode, hoveredModeInfo, aiMode, typingStatus, guideText }) {
  ...
  const typingStates = { listening: { label: "LISTENING", icon: "idle", bg: "#1A2019", text: S.lcd }, weak: { label: "PUSH MORE", icon: "thinking", bg: "#2A1E17", text: S.accent }, vague: { label: "BE SPECIFIC", icon: "thinking", bg: "#2A1E17", text: S.accent }, strong: { label: "LOOKING GOOD", icon: "approves", bg: "#1A2219", text: S.lcdBright }, composing: { label: "COMPOSING", icon: "cocreate", bg: "#1A2019", text: S.lcd } };
  let es; if (isThinking) es = { label: "READING", icon: "thinking", bg: "#2E1E14", text: S.accent }; else if (mode === "approves") es = { label: "NOTED", icon: "approves", bg: "#162216", text: S.lcdBright }; else if (typingStatus) es = typingStates[typingStatus] || typingStates.listening; else es = { label: idleLabels[aiMode] || "READY", icon: idleIcons[aiMode] || "idle", bg: "#1A2019", text: S.lcd }; if (mode === "off") es = { label: "LUCY", icon: "off", bg: S.screen, text: S.lcdDim };
  const gridRgb = es.text === S.accent ? "212,115,74" : es.text === S.lcdBright ? "160,200,160" : "122,154,122";
  const smi = hoveredModeInfo && isActive; const dIcon = smi ? (hoveredModeInfo.key === "GDE" ? "guide" : hoveredModeInfo.key === "CRT" ? "cocreate" : "idle") : es.icon; const dLabel = smi ? hoveredModeInfo.desc : es.label; const dColor = smi ? S.lcd : es.text; const dBg = smi ? "#1A2019" : es.bg;
  return (
...
function ActionBar({ hasText, onSpark, onKeep }) {
  ...
  return (<button onMouseDown={hasText ? () => { setPressed(true); setTimeout(() => { onKeep(); setPressed(false); }, 120); } : () => { if (fired) return; setPressed(true); setFired(true); setTimeout(() => { onSpark(); setPressed(false); }, 250); }} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "11px 0", borderRadius: 0, border: "none", cursor: "pointer", fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: hasText ? (pressed ? "rgba(61,56,48,0.4)" : S.text) : (pressed ? "rgba(212,115,74,0.4)" : "rgba(212,115,74,0.6)"), background: pressed ? `linear-gradient(180deg, ${S.recess} 0%, #CCC7BD 100%)` : `linear-gradient(180deg, #F2EDE5 0%, ${S.card} 100%)`, boxShadow: pressed ? "0 1px 3px rgba(0,0,0,0.06) inset" : "0 -1px 0 rgba(0,0,0,0.03), 0 1px 0 rgba(255,255,255,0.6) inset", transform: pressed ? "translateY(0.5px)" : "translateY(0)", transition: "all 0.06s ease" }}>{hasText ? (<><span style={{ fontSize: 8, fontWeight: 500, opacity: 0.3 }}>⌘ ENTER</span><span>{pressed ? "KEEPING..." : "KEEP ↵"}</span></>) : (<><div style={{ width: 6, height: 6, borderRadius: "50%", background: pressed ? "rgba(212,115,74,0.3)" : S.accent }} /><span>{pressed ? "SPARKING..." : "SPARK"}</span></>)}</button>);
}

function Btn({ children, onClick, accent, style: sx }) {
  const [p, sP] = useState(false);
  return (<button onClick={onClick} onMouseDown={() => sP(true)} onMouseUp={() => sP(false)} onMouseLeave={() => sP(false)} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 10, fontWeight: 600, fontFamily: "'Inter', sans-serif", letterSpacing: "0.08em", textTransform: "uppercase", padding: "7px 14px", borderRadius: 3, border: "none", cursor: "pointer", color: accent ? (p ? "#EDE9E1" : S.accent) : (p ? "#EDE9E1" : S.text), background: accent ? (p ? "#B85A35" : S.card) : (p ? S.text : S.card), boxShadow: p ? S.pressed : S.raised, transform: p ? "translateY(1px)" : "translateY(0)", transition: "all 0.08s ease", ...sx }}>{children}</button>);
}
...
function PanelCard({ note, onEdit }) {
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div onClick={() => !exp && sExp(true)} onMouseEnter={() => sH(true)} onMouseLeave={() => sH(false)} style={{ background: S.card, borderRadius: 4, border: `1px solid ${exp ? "rgba(212,115,74,0.12)" : "rgba(61,56,48,0.08)"}`, boxShadow: h || exp ? "0 2px 8px rgba(0,0,0,0.06)" : "0 1px 3px rgba(0,0,0,0.03)", transform: h && !exp ? "translateY(-1px)" : "none", transition: `all 0.25s ${ease}`, cursor: exp ? "default" : "pointer", overflow: "hidden" }}>
        <div style={{ height: 2, background: h || exp ? S.accent : "rgba(61,56,48,0.06)", transition: "background 0.25s ease" }} />
        <div style={{ padding: "12px 14px 10px" }}>
          {!exp ? (<><div style={{ fontSize: 13, fontWeight: 400, lineHeight: 1.65, color: S.text }}>{note.text}</div><div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}>{note.lucyMode && <span style={{ fontSize: 8, fontWeight: 700, letterSpacing: "0.06em", padding: "1px 4px", borderRadius: 2, background: note.lucyMode === "cocreate" ? "rgba(122,154,122,0.15)" : "rgba(212,115,74,0.1)", color: note.lucyMode === "cocreate" ? S.lcd : "rgba(212,115,74,0.5)" }}>{MODES[note.lucyMode]?.key}</span>}<span style={{ fontSize: 9, fontWeight: 600, letterSpacing: "0.05em", color: "rgba(61,56,48,0.14)", textTransform: "uppercase" }}>{note.prompt}</span></div></>) : (<><textarea ref={eRef} value={et} onChange={(e) => sEt(e.target.value)} style={{ width: "100%", minHeight: 60, background: "transparent", border: "none", fontSize: 13, fontWeight: 400, lineHeight: 1.65, color: S.text, resize: "none", outline: "none", fontFamily: "'Inter', sans-serif" }} onKeyDown={(e) => { if (e.key === "Enter" && e.metaKey) { onEdit(et); sExp(false); } if (e.key === "Escape") { sEt(note.text); sExp(false); } }} /><div style={{ display: "flex", justifyContent: "flex-end", gap: 4, marginTop: 6, paddingTop: 6, borderTop: `1px solid ${S.border}` }}><Btn onClick={(e) => { e.stopPropagation(); sEt(note.text); sExp(false); }}>ESC</Btn><Btn accent onClick={(e) => { e.stopPropagation(); onEdit(et); sExp(false); }}>SAVE</Btn></div></>)}
        </div>
        {note.lucyText && (<div style={{ opacity: lucyVis ? 1 : 0, transition: `opacity 0.4s ${ease}` }}><div style={{ height: 1, background: note.lucyMode === "cocreate" ? "rgba(122,154,122,0.12)" : "rgba(212,115,74,0.08)" }} /><div style={{ padding: "8px 14px", display: "flex", alignItems: "flex-start", gap: 8 }}><div style={{ width: 4, height: 4, borderRadius: "50%", marginTop: 4, flexShrink: 0, background: note.lucyMode === "cocreate" ? S.lcd : S.accent }} /><div style={{ fontFamily: "'DotGothic16', monospace", fontSize: 10, color: note.lucyMode === "cocreate" ? "rgba(122,154,122,0.6)" : "rgba(212,115,74,0.55)", lineHeight: 1.5 }}>{note.lucyText}</div></div></div>)}
      </div>
    </div>
  );
}
    ...
    <div style={{ height: "100vh", overflow: "hidden", fontFamily: "'Inter', sans-serif", color: S.text, position: "relative", background: "#D8D3CA" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .panel-scroll::-webkit-scrollbar-thumb { background:rgba(61,56,48,0.06); border-radius:3px; }
      `}</style>

      {/* Canvas */}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, #ECE7DE 0%, #E3DED4 100%)", transition: `transform 0.45s ${ease}`, transform: panelOpen ? `translateX(-${PANEL_W}px)` : "translateX(0)", overflow: "hidden" }}>
        {/* Header */}
        <div style={{ padding: "6px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${S.border}`, background: S.panel, boxShadow: "0 1px 0 rgba(255,255,255,0.4) inset, 0 1px 3px rgba(0,0,0,0.02)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 10px", borderRadius: 4, height: 24, background: "rgba(61,56,48,0.04)", boxShadow: "0 1px 2px rgba(0,0,0,0.03) inset, 0 1px 0 rgba(255,255,255,0.5)" }}>
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(61,56,48,0.35)" }}>Personality</span>
              <span style={{ fontSize: 9, color: "rgba(61,56,48,0.35)" }}>–</span>
              <span style={{ fontSize: 9, fontWeight: 400, letterSpacing: "0.06em", textTransform: "uppercase", color: "rgba(61,56,48,0.35)" }}>{done ? "Complete" : prompt?.chapter}</span>
            </div>
            {prompt && (
            ...
            <button onClick={() => { if (!done) { setCur((c) => c + 1); setAnimKey((k) => k + 1); setText(""); } }} style={{ width: 28, height: 28, borderRadius: 4, border: "none", cursor: done ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(61,56,48,0.04)", boxShadow: "0 1px 2px rgba(0,0,0,0.03) inset, 0 1px 0 rgba(255,255,255,0.5)", color: done ? "rgba(61,56,48,0.08)" : "rgba(61,56,48,0.25)", fontSize: 14, fontFamily: "'Inter', sans-serif", transition: "all 0.08s ease" }}>›</button>
          </div>
        </div>
          ...
          {!done && prompt && (
            <div style={{ padding: "10vh 48px 60px", display: "flex", justifyContent: "center" }}>
              <div key={animKey} style={{ width: "100%", maxWidth: 520, animation: `promptIn 0.4s ${ease} both` }}>
                ...
                <p style={{ fontSize: 12, fontWeight: 400, color: "rgba(61,56,48,0.3)", marginBottom: 24, lineHeight: 1.5 }}>{prompt.sub}</p>
                <div>
                  <div style={{ background: S.recess, borderRadius: "6px 6px 0 0", border: `1px solid ${S.border}`, borderBottom: "none", boxShadow: "0 2px 6px rgba(0,0,0,0.04) inset, 0 1px 2px rgba(0,0,0,0.02) inset", overflow: "hidden" }}>
                    <div style={{ padding: "20px 22px 16px" }}>
                      <textarea ref={ref} value={text} onChange={(e) => setText(e.target.value)} placeholder="Start writing..." rows={5} style={{ width: "100%", minHeight: 120, background: "transparent", border: "none", padding: 0, fontSize: 16, fontWeight: 400, lineHeight: 1.8, color: S.text, resize: "none", fontFamily: "'Inter', sans-serif", letterSpacing: "-0.01em" }} onKeyDown={(e) => { if (e.key === "Enter" && e.metaKey) keep(); }} />
                    </div>
          ...
          {done && (
            <div style={{ padding: "20vh 48px", display: "flex", justifyContent: "center" }}>
              <div style={{ maxWidth: 520, width: "100%", textAlign: "center", animation: `promptIn 0.5s ${ease} both` }}>
                <div style={{ display: "inline-block", fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "4px 10px", borderRadius: 3, background: S.accent, color: "#EDE9E1", marginBottom: 16 }}>COMPLETE</div>
                <h2 style={{ fontSize: 26, fontWeight: 300, marginBottom: 8, lineHeight: 1.4 }}>A character is forming.</h2>
                <p style={{ fontSize: 13, fontWeight: 400, color: "rgba(61,56,48,0.3)", lineHeight: 1.6 }}>Nine thoughts across two chapters. The outline of someone real.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Drawer */}
      <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: PANEL_W, background: S.panel, borderLeft: "1px solid rgba(61,56,48,0.08)", boxShadow: panelOpen ? "-4px 0 24px rgba(0,0,0,0.06)" : "none", transform: panelOpen ? "translateX(0)" : `translateX(${PANEL_W}px)`, transition: `transform 0.45s ${ease}`, zIndex: 20, display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${S.border}`, display: "flex", alignItems: "center", gap: 8, flexShrink: 0, boxShadow: "0 1px 0 rgba(255,255,255,0.3) inset" }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 600, color: "rgba(61,56,48,0.12)", fontVariantNumeric: "tabular-nums" }}>{notes.length}</span>
        </div>
        <div className="panel-scroll" style={{ flex: 1, overflowY: "auto", padding: "12px 12px 80px" }}>
          {notes.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px", color: "rgba(61,56,48,0.1)", fontSize: 12 }}>Your thoughts will appear here.</div>
      ...
      {/* Drawer toggle */}
      <button onClick={() => setPanelOpen((o) => !o)} style={{ position: "absolute", right: panelOpen ? PANEL_W : 0, top: "50%", transform: "translateY(-50%)", width: 36, height: 72, borderRadius: "6px 0 0 6px", background: panelOpen ? S.accent : S.screen, border: `1px solid ${panelOpen ? "rgba(212,115,74,0.4)" : "rgba(0,0,0,0.12)"}`, borderRight: "none", boxShadow: panelOpen ? "-2px 0 12px rgba(212,115,74,0.15), 0 1px 0 rgba(255,255,255,0.15) inset" : "-2px 0 8px rgba(0,0,0,0.04), 0 1px 3px rgba(0,0,0,0.12) inset, 0 1px 0 rgba(255,255,255,0.04)", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, zIndex: 25, transition: `right 0.45s ${ease}, background 0.3s ease, border-color 0.3s ease, box-shadow 0.3s ease` }}>
